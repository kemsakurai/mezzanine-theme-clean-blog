    ...
    methods: {
      init: function( options ) {
        ...
        $(uniqueForms).bind('submit', function(e) {
          let $form = $(this);
          let warningsFound = 0;
          let $inputs = $form.find('input,textarea,select').not('[type=submit],[type=image]').filter(settings.options.filter);
      ...
      ajax: {
        ...
        blockSubmit: true,
			...
			maxchecked: {
				name: 'maxchecked',
				init: function($this, name) {
					let elements = $this.parents('form').first().find('[name="' + $this.attr('name') + '"]');
