...
var WorkboxSW = (function () {
    ...
    class LogGroup {
        ...
        print() {
            return 0 === this._logs.length && 0 === this._childGroups.length ? void this._printLogDetails(this._primaryLog) : void(this._primaryLog && (this._isFallbackMode ? this._printLogDetails(this._primaryLog) : console.groupCollapsed(...this._getLogContent(this._primaryLog))), this._logs.forEach((a) = > {this._printLogDetails(a);
        }),
            this._childGroups.forEach((a) = > {a.print();
        }),
    ...
    class LogHelper {
        ...
        _getAllLogGroups(a, b) {
            ...
            return b.data && ('object' != typeof b.data || b.data instanceof Array ? e.addLog(this._getKeyValueDetails('additionalData', b.data)) : Object.keys(b.data).forEach((a) = > {e.addLog(this._getKeyValueDetails(a, b.data[a]));
        })),
            c.addChildGroup(e), c
        }
    ...
    function atLeastOne(a) {
        ...
        throwError('Please set at least one of the following parameters: ' + b.map((a) = > `'${a}'`).join(', ')
    )
        ;
    }
    ...
    class ExpressRoute extends Route {
        constructor({path: a, handler: b, method: c}) {
            ...
            let d = [];
            const e = index(a, d);
            super({match: ({url: b}) = > {if(a.startsWith('/') && b.origin !== location.origin)
            return null;
            const c = a.startsWith('/') ? b.pathname : b.href, f = c.match(e);
            if (!f) return null;
            const g = {};
            return d.forEach((a, b) = > {g[a.name] = f[b + 1];
        }),
            g
        },
            handler:b, method
    ...
    class NavigationRoute extends Route {
        constructor({whitelist: a, blacklist: b, handler: c} = {}) {
            ...
            super({match: ({event: d, url: e}) = > {let f, g = !1;
            if ('navigate' === d.request.mode) {
                ...
                a.some((a) = > a.test(d)
                    ...
                    ? f = `The navigation route is not being used, since the ` + `request URL matches both the whitelist and blacklist.` : (f = `The navigation route is being used.`, g = !0)
            :
                f = `The navigation route is not being used, since the ` + `URL being navigated to doesn't match the whitelist.`, logHelper.debug({
                    that: this,
                    message: f,
    ...
    class RegExpRoute extends Route {
        constructor({regExp: a, handler: b, method: c}) {
            ...
            super({match: ({url: b}) = > {const c = a.exec(b.href);
    ...
    class Router$2 {
        ...
        addFetchListener() {
            ...
            }), !1) : (this._isListenerRegistered = !0, self.addEventListener('fetch', (a) = > {const b = this.handleRequest({event: a});
            b && a.respondWith(b);
        }),
            !0
        )
        ...
        handleRequest({event: a}) {
            ...
            if (!c && this.defaultHandler && (c = this.defaultHandler), c) {
                ...
                return this.catchHandler && (e = e.catch((c) = > this.catchHandler.handle({url: b, event: a, error: c}))
            ),
                e
            }
        }
    ...
    class Router$$1 extends Router$2 {
        ...
        registerNavigationRoute(a, b = {}) {
            ...
            super.registerRoute({route: new NavigationRoute({handler: () = > caches.match(a, {cacheName: c}), whitelist
        :
    ...
    class CacheableResponse {
        ...
        isResponseCacheable({request: a, response: b} = {}) {
            ...
            let c = !0;
            if (this.statuses && (c = this.statuses.includes(b.status)), this.headers && c && (c = Object.keys(this.headers).some((a) = > b.headers.get(a) === this.headers[a])),
            !c
        )
            {
                const c = {response: b};
...
>
    ...
    var cleanResponseCopy = (({response: a}) = > {isInstance({response: a}, Response);
        ...
        expireEntries({cacheName: a, now: b} = {}) {
            ...
            return asyncToGenerator(function* () {
                ...
                const d = c.maxAgeSeconds ? yield c.findOldEntries({cacheName: a, now: b}) : [],
                    e = c.maxEntries ? yield c.findExtraEntries({cacheName: a}) : [], f = [...new
                Set(d.concat(e))
            ]
                ;
    ...
    function broadcastUpdate({channel: a, cacheName: b, url: c, source: d} = {}) {
        'BroadcastChannel' in self && (isInstance({channel: a}, BroadcastChannel), isType({cacheName: b}, 'string'), isType({source: d}, 'string'), isType({url: c}, 'string'), a.postMessage({
    ...
    function responsesAreSame({first: a, second: b, headersToCheck: c} = {}) {
        ...
        ;
        return d ? c.every((c) = > a.headers.has(c) === b.headers.has(c) && a.headers.get(c) === b.headers.get(c)
    ):
        (logHelper.log({
    ...
    class BroadcastCacheUpdate {
        ...
        get channel() {
            return this._channel || ('BroadcastChannel' in self ? this._channel = new BroadcastChannel(this.channelName) : this._channel = {postMessage: () = > {}
        }),
            this._channel
        }
    ...
    class Strategies {
        ...
        _getCachingMechanism(a, b = {}) {
            ...
            b.plugins && b.plugins.forEach((a) = > {d.plugins.push(a);
        }),
...
>
    ...
    class BaseCacheManager {
        ...
        _addEntries(a) {
            this._parsedCacheUrls = null, a.forEach((a) = > {this._addEntryToInstallList(this._parseEntry(a));
        })
            ;
        }
        ...
        getCachedUrls() {
            return this._parsedCacheUrls || (this._parsedCacheUrls = Array.from(this._entriesToCache.keys()).map((a) = > new URL(a, location).href)
        ),
    ...
    class IDBHelper {
        ...
        _getDb() {
            return this._dbPromise ? this._dbPromise : (this._dbPromise = idb.open(this._name, this._version, (a) = > {a.createObjectStore(this._storeName);
        }).
            then((a) = > a
        ),
            this._dbPromise
        ...
        close() {
            return this._dbPromise ? this._dbPromise.then((a) = > {a.close(), this._dbPromise = null;
        }):
            void 0
        }
    ...
    class RevisionedCacheManager extends BaseCacheManager {
        ...
        install() {
            return super.install().then((a) = > {const b = [], c = [];
            a.forEach((a) = > {a.wasUpdated ? b.push({url: a.url, revision: a.revision}) : c.push({
                url: a.url,
                revision: a.revision
            });
        })
    ...
    class WorkboxSW$1 {
        ...
        _registerInstallActivateEvents(a, b) {
            self.addEventListener('install', (b) = > {const c = this._revisionedCacheManager.getCachedUrls();
            ...
            }), b.waitUntil(this._revisionedCacheManager.install().then(() = > {if(a)return self.skipWaiting()})
        )
            ;
        }),
            self.addEventListener('activate', (a) = > {a.waitUntil(this._revisionedCacheManager.cleanup().then(() = > {
                if(b)
        ...
        _registerDefaultRoutes(a, b) {
            ...
            this._precacheRouter.registerRoute(({url: c}) = > {c.hash = '';
